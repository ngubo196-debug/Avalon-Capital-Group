  ...
  return (
    <section className="ada-hero-massive">
      <style suppressHydrationWarning>{`
        .ada-hero-massive { position: relative; min-height: 100vh; background: #FFFFFF; display: flex; align-items: center; justify-content: flex-start; padding: 68px 3.5rem 0; overflow: hidden; }
        .ada-hero-massive-inner { position: relative; z-index: 2; max-width: 1160px; margin: 0 auto; width: 100%; }
        .ada-hero-massive-h1 { font-family: 'Bodoni Moda', 'Cormorant Garamond', serif; font-weight: 400; font-size: clamp(72px, 9vw, 128px); line-height: 1.0; letter-spacing: -0.03em; color: #111111; margin: 0; }
        .ada-hero-massive-h1 em { font-style: italic; color: #1B3A8F; }
        .ada-hero-massive-ctas { display: flex; align-items: center; gap: 24px; flex-wrap: wrap; margin-top: 3rem; }
        .ada-hero-massive-primary { display: inline-flex; align-items: center; background: #1B3A8F; color: #FFFFFF; border-radius: 100px; padding: 14px 32px; font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 500; text-decoration: none; transition: opacity 0.2s ease, transform 0.2s ease; }
        .ada-hero-massive-primary:hover { opacity: 0.88; transform: translateY(-1px); }
        .ada-hero-massive-ghost { font-family: 'DM Sans', sans-serif; font-size: 14px; color: #7A7A7A; text-decoration: none; border-bottom: 1px solid #E2E2DC; padding-bottom: 2px; transition: color 0.2s ease, border-color 0.2s ease; }
        .ada-hero-massive-ghost:hover { color: #111111; border-color: #111111; }
        .ada-hero-massive-scroll { position: absolute; bottom: 32px; left: 50%; transform: translateX(-50%); z-index: 2; display: flex; flex-direction: column; align-items: center; gap: 8px; color: #7A7A7A; font-family: 'DM Sans', sans-serif; font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; }
